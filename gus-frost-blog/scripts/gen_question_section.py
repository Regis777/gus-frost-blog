...
{%- if search.performed -%}
  ...
  <script>
    /* Klaviyo en second canal. Ne doit JAMAIS empecher l'envoi du formulaire. */
    (function () {
      ...
      function inscrit(email) {
        if (!cfg.liste) return Promise.resolve();
      ...
      form.addEventListener('submit', function (e) {
        if (parti) return;                     /* 2e passage : on laisse filer */
        ...
        var taches = [evenement(champEmail.value, champTexte.value)];
        if (optin && optin.checked) taches.push(inscrit(champEmail.value));

        var fini = false;
        function suite() { if (fini) return; fini = true; form.submit(); }
        setTimeout(suite, 2000);               /* filet : l'e-mail part quoi qu'il arrive */
        Promise.all(taches)
