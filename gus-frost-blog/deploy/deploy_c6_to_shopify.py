...
# -*- coding: utf-8 -*-
"""
Déploie le cluster C6 (chiot) dans le blog « Chiens » de Gus & Frost, EN BROUILLON,
sur le modèle C5 — avec deux spécificités C6 :
...
Usage :
  python deploy/deploy_c6_to_shopify.py --dry-run   # aucune écriture : diagnostic + récap
  python deploy/deploy_c6_to_shopify.py --bake      # upload images + crée les 13 brouillons
"""
import argparse
import csv
import os
...
ARTDIR = os.path.join(ROOT, "articles", "cluster-6-chiot")
IMG_DIR = os.path.join(ROOT, "images", "cluster-6-chiot")   # 22 .png renommés (comme images/cluster-5)
MAP = os.path.join(ARTDIR, "C6_images_map.csv")
...
def c6_rows():
    rows = [r for r in read_manifest() if r["cluster_num"] == "6"]
...
def main():
    ...
    print("=" * 74)
    print("C6 (chiot) -> blog «%s» | brouillon | %s" % (env["BLOG_HANDLE"], mode))
    print("Images : %d au total | déjà sur Files : %d | absentes localement : %d"
          % (len(all_files), len(cdn_map), len(local_missing)))
    if local_missing:
        print("  ⚠ à décompresser (images_C6_renamed.zip) avant exécution : %d fichiers, ex. %s"
